...
public class MyMainManagement2 {
		...
		// Cositas que vamos a ir necesitando
				...
				//Vamos a comprobar que se ha creado correctamente uno y otro
						try {
						    ...
						    //Imprimimos la ruta del archivo y hacemos un 2x1, de esta forma comprobamos que esta
				...
				//Ya que tenemos el archivo creado, vamos a escribir en él. El cómo, va a ser usando el método de copia
						copiar_documento (origen,datos);
						lectura_doc (origen,datos);
						cont_words (datos);
						permisos (origen);
						permisos ();
						mover(origen);
...
//_______METODOS CUSTOM___________________________________________________________________________________________________________________
	...
	public static void cont_words (File archivo) {
		try {
			...
			//Vamos a ir recorriendo el documento línea a línea hasta que sea null (es decir, que no haya niguna más
	...
	//Método de lectura de archivo. Hemos optado por que al usuario se le muestre el original o la copia, de modo que sea
	 ...
	 public static void mover (File origen) {
	    ...
	    //dónde está y que todo es como debe
		 ...
		 try {
	        	...
	        	System.out.println("El archivo no se ha podido mover");
	            ex.printStackTrace();
	        }
	 }
}